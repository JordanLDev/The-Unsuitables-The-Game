...
namespace Game1
{
    public class NeddyAnimatedSprite
    {
        ...
        public NeddyAnimatedSprite(Texture2D texture, Rectangle player2Rect, int rows, int columns)
        {
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 location2)
        {
            ...
            Rectangle destinationRectangle = new Rectangle((int)location2.X, (int)location2.Y, width, height);
            ...
            if (ks.IsKeyDown(Keys.D) && location2.X < 512 || location2.X < 512 || (ks.IsKeyUp(Keys.D) && ks.IsKeyUp(Keys.A) && location2.X < 512))
            {
                spriteBatch.Draw(neddySpriteTexture, destinationRectangle, sourceRectangle, Color.White, 0, Vector2.Zero, SpriteEffects.None, 0);
            }
            else
            if (ks.IsKeyDown(Keys.A) && location2.X > 512 || location2.X > 512 || (ks.IsKeyUp(Keys.D) && ks.IsKeyUp(Keys.A) && location2.X > 512))
            {
                spriteBatch.Draw(neddySpriteTexture, destinationRectangle, sourceRectangle, Color.White, 0, Vector2.Zero, SpriteEffects.FlipHorizontally, 0);
            }
        }
